...
export class PointsExpiryJob {
  ...
  /** Runs every 5 minutes ΓÇö expires batches whose expiryDate has passed (set to now + configured window on transaction). */
  ...
  private async sendExpiryWarnings(
    ...
    for (const row of expiringRows) {
      try {
        // WhatsApp warning only ΓÇö email is sent on actual expiry, not during warnings
...
<body style="margin:0;padding:0;background:#f4f4f5;font-family:Arial,Helvetica,sans-serif;">
  ...
  private async expirePoints() {
    ...
    // Only process batches with pointsRemaining > 0 ΓÇö batches fully consumed by
    ...
    for (const row of expiredRows) {
      ...
      try {
        await this.prisma.$transaction(async (tx: Prisma.TransactionClient) => {
          ...
          // Deduct only pointsRemaining ΓÇö the portion never consumed by redemptions
