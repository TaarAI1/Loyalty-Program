    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          ...
          <div className="border rounded-md p-4 bg-white text-sm font-sans space-y-3">
            <p className="text-red-600 font-bold text-base">🚨 Suspicious Activity Detected</p>
            <table style={{ borderCollapse: 'collapse', width: '100%' }} className="text-xs">
              <tbody>
                {[
                  ['Customer Name', '[Customer Name — from database]'],
                  ['Mobile Number', '[Mobile Number — from database]'],
                  ...
                  <tr key={label}>
                    <th style={{ background: '#f3f4f6', border: '1px solid #d1d5db', padding: '6px 10px', textAlign: 'left', whiteSpace: 'nowrap' }}>{label}</th>
                    <td style={{ border: '1px solid #d1d5db', padding: '6px 10px', color: '#6b7280' }}>{value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <p className="text-[11px] text-muted-foreground">Generated by LoyaltyPlus Forensic Monitor</p>
          </div>
          <p className="text-xs text-muted-foreground mt-1">
            This email is sent automatically to the Forensic Alert Email address when a customer has 5 or more transactions within 3 days.
            Customer name and mobile number are filled dynamically from the database.
          </p>
        </div>
